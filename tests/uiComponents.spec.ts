...
test.describe('Form layouts page', ()=> {
    ...
    //locator assertion
        ...
        const radioButtonTwo = usingTheGridForm.getByRole('radio', {name: "Option 2"})
        await radioButtonTwo.check({force: true})
...
test('checkboxes', async({page})=> {
    ...
    // method .all pretvara u array. Zelimo chekirati svaki chekbox i uraditi assertion da je svaki chekiran
...
test('date pickers', async({page})=> {
    ...
    let calenarMonthAndYear = await page.locator('nb-calendar-view-mode').textContent()
    const expectedMonthAndYear = `${expectedMonthLong} ${expectedYear}`
    while(!calenarMonthAndYear.includes(expectedMonthAndYear)) {
        await page.locator('nb-calendar-pageable-navigation [data-name="chevron-right"]').click()
        calenarMonthAndYear = await page.locator('nb-calendar-view-mode').textContent()
...
test('sliders', async({page})=>{
    ...
    await tempGauge.evaluate( node => {
        node.setAttribute('cx', '216.604')
        node.setAttribute('cy', '39.460')
    })
